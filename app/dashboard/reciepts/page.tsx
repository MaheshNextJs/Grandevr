...
/* ── Types & mock data (same shape/feel as dashboard, tweaked for receipts) ── */
type Receipt = {
  id: string;
  hotel: string;
  checkin: string;
...
/* ── Bits (same look as your dashboard bits) ── */
function Breadcrumb() {
  return (
    ...
    <aside className="rounded-lg border border-gray-200 shadow-lg bg-white">
      <div className="p-4 space-y-3 text-sm text-gray-700">
        <div className="flex items-center gap-2">
          <Image src="/icons/user2.png" alt="User" width={15} height={15} />
          <span>John Doe</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <button className="mt-2 flex mx-auto items-center gap-2 text-xs border border-gray-300 shadow-sm rounded-md px-3 py-2 text-white bg-[#A57865] hover:bg-[#7a3c23]">
...
/* ── Card (same layout as dashboard card, with 'Status' + 'View Receipt') ── */
function ReceiptCard({ r }: { r: Receipt }) {
  return (
    <div className="border border-gray-200 rounded-lg bg-white shadow-sm">
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between">
            <div>
              ...
              <div className="text-gray-900">{r.id.split("-")[0]}</div>
            </div>
            <div className="text-right">
              <div className="text-[11px] text-gray-500">Hotel</div>
              <div className="text-gray-900">{r.hotel}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="text-gray-900">{r.checkin}</div>
            </div>
            <div>
              <div className="text-[11px] text-gray-500">Check-out</div>
              <div className="text-gray-900">{r.checkout}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="text-gray-900">{r.room}</div>
            </div>
            <div>
              <div className="text-[11px] text-gray-500">Guests</div>
              <div className="text-gray-900">{r.guests}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div
              className={`inline-flex items-center gap-1 ${
                r.status === "Completed" ? "text-green-600" : "text-gray-600"
              }`}
            >
      ...
      {/* Buttons row (right-aligned like your dashboard) */}
...
/* ── Page (same structure as dashboard) ── */
export default function ReceiptsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-25">
      {/* Title row (match your dashboard spacing) */}
      <div className="flex items-center justify-between">
        <h1 className="text-xl md:text-2xl font-semibold">Receipts</h1>
      </div>
      ...
      {/* Downloads aligned to right (same as your dashboard) */}
      <div className="flex items-center justify-between">
        ...
        <div className="lg:col-span-3">
          ...
          <div className="space-y-4">
            {list.length === 0 ? (
              ...
              list.map((r) => <ReceiptCard key={r.id} r={r} />)
            )}
          </div>
        </div>
      </div>
