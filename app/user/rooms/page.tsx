...
import Image from "next/image";
import Link from "next/link";
import { FEATURE_ICONS, ROOMS } from "./data";
          ...
          >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-gray-700 text-sm">
                  {room.features.map((feature, i) => (
                    <span key={i} className="flex items-center gap-2">
                      <Image
                        src={FEATURE_ICONS[feature] || "/icons/default.png"}
                        alt={feature}
                        width={14}
                        height={14}
                      />
            ...
            <div className="sm:w-1/4 flex flex-col justify-center items-center p-6 text-center">
              ...
              <Link
                ...
                className="mt-4 inline-block border border-[#5A2A17] text-[#5A2A17] px-4 py-2 rounded-sm text-sm hover:bg-[#f5ece8] transition duration-200 active:scale-95"
              >
                View Details
              </Link>
            </div>
